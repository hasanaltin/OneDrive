...
It also detects your package manager (apt/dnf/pacman/zypper) and installs whatever system
packages are actually missing - FUSE 3, and (only if no prebuilt Python wheel is available for
your Python version) the C compiler, `pkg-config`, and `libfuse3`/Python headers needed to build
`pyfuse3` from source - so expect a `sudo` password prompt the first time if any of those aren't
already on your system. If it finishes without a registered Azure app yet, it'll ask right there
whether to run `register_azure_app.sh` (below) for you.
